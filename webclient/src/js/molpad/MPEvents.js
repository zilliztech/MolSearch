...
MolPad.prototype.mouseDragHandler = {
	...
	onPointerMove: function (e, mp) {
		...
		mp.translate(
			(p.x - mp.pointer.old.p.x) * mp.devicePixelRatio,
...
MolPad.prototype.multiTouchHandler = {
	onPointerMove: function (e, mp) {
		...
		mp.translate(
			(c.x - mp.pointer.old.c.x) * mp.devicePixelRatio,
...
MolPad.prototype.selectionToolHandler = {
	...
	onDraw: function (mp) {
		...
		mp.ctx.setLineDash([2 / mp.getScale(), 5 / mp.getScale()]);
		...
		if (mp.tool.data.type === "rect" && this.data.rect) {
			mp.ctx.rect(
				this.data.rect.x,
				this.data.rect.y,
				this.data.rect.width,
				this.data.rect.height
			);
	...
	if (this.tool.type === "atom") {
		...
		return {
			onPointerDown: function (e, mp) {
				...
				mp.sel.currentAngle = mp.sel.startAngle = 0.5 * Math.PI;
			},
			onPointerMove: function (e, mp) {
				var p = new MPPoint().fromRelativePointer(e, mp);
				mp.sel.rotate(p);
		...
		return {
			onPointerDown: function (e, mp) {
				...
				var frag = MPFragments.translate(
					MPFragments.scale(
						MPFragments.clone(mp.tool.data.frag.full),
						mp.s.bond.length
					),
					p.x,
					p.y
