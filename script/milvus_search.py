...
TOFILE = False
# Randomly search from GT_NQ query vectors
GT_NQ = 100
...
def search_vec_list(table_name, nq, topk, nprobe):
    rand = None
    query_list = []
    # vectors = load_uint8_vec()
    # vectors = load_all_vec()
    vectors = load_hex_vec()
    if nq != 0:
        try:
    ...
    status, results = MILVUS.search(collection_name=table_name, query_records=query_list, top_k=topk, params={"nprobe":nprobe})
    #print(status)
    time_end = time.time()
    ...
    for opt_name, opt_value in opts:
        if opt_name in ("-h", "--help"):
            ...
            if TOFILE:
                ...
                print("get the search smiles success.")
            sys.exit()
