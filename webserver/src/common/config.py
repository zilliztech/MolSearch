import os

MILVUS_HOST = os.getenv("MILVUS_HOST", "192.168.1.85")
MILVUS_PORT = os.getenv("MILVUS_PORT", 19532)
VECTOR_DIMENSION = os.getenv("VECTOR_DIMENSION", 2048)
NUM = os.getenv("TOPK_NUM", 100)
DEFULT_TABLE = os.getenv("DEFULT_TABLE", "milvus_mols_10")


PG_HOST = os.getenv("PG_HOST", "192.168.1.85")
PG_PORT = os.getenv("PG_PORT", 5432)
PG_USER = os.getenv("PG_USER", "milvus")
PG_PASSWORD = os.getenv("PG_PASSWORD", "milvus_postgres")
PG_DATABASE = os.getenv("PG_DATABASE", "milvus")
PG_TABLE = os.getenv("PG_TABLE", "milvus_mols")